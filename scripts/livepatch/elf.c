...
//FIXME should they just all be rebuilt?!?!?!
//FIXME have a elf_write() and elf_write_file()
//FIXME cleanup list.h?
...
static int update_symtab(struct elf *elf)
{
	...
	symtab->elf_data->d_size = symtab->size = size; //FIXME need to update shdr????
...
static int write_file(struct elf *elf, const char *file)
{
	...
	list_for_each_entry(sec, &elf->sections, list) {
		...
		if (!elf_flagdata(data, ELF_C_SET, ELF_F_DIRTY)) {
			WARN("elf_flagdata failed"); //FIXME better elf-specific WARN
			return -1;
		}
...
int elf_write(struct elf *elf, const char *file)
{
	int ret;
